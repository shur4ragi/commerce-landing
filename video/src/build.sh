...
python3 clips.py
node render.mjs video                                                  # motion graphics quadro a quadro (30 fps)
...
ffmpeg -hide_banner -loglevel error -y -i video_noaudio.mp4 -i music.wav -map 0:v -map 1:a \
  ...
  -c:a aac -b:a 192k -shortest "$ROOT/video/aurora-cafe-apresentacao.mp4"
echo "OK → video/aurora-cafe-apresentacao.mp4"
